...
| Var | Purpose |
|-----|---------|
| `SECRET_KEY` | Signs the session cookie. **Required** — the app refuses to start without a real value. Generate: `python -c "import secrets; print(secrets.token_hex(32))"`. |
| `APP_USERNAME` | Login username. |
| `APP_PASSWORD_HASH` | bcrypt hash of the login password (see above). Empty → no one can log in. |
| `DATABASE_URL` | SQLite URL, e.g. `sqlite:///./shop.db`. |
| `UPLOAD_DIR` | Where job file attachments are stored (default `./uploads`). |
| `SESSION_HTTPS_ONLY` | `1` (default) sets the `Secure` cookie flag; use `0` only for local HTTP dev. |
...
3. **Persist the `/data` volume** — it holds the SQLite DB
   (`DATABASE_URL=sqlite:////data/shop.db`), job file attachments
   (`UPLOAD_DIR=/data/uploads`), and backups. A single Uvicorn worker is enough.
   The image runs as a non-root user, exposes `/healthz` for platform health
   checks, and enables SQLite WAL mode for safe concurrent reads/backups.
...
# Restore (verifies integrity, saves the current DB aside; stop the app first).
# Restores the DB and, optionally, the matching uploads archive:
scripts/restore.sh /data/backups/shop-YYYYMMDD-HHMMSS.db.gz /data/shop.db \
                   /data/backups/uploads-YYYYMMDD-HHMMSS.tgz /data
```
Backups include both the database **and** uploaded files. A restore→row-count
round-trip runs in CI (`tests/test_backup_restore.py`); still verify a real
restore into a scratch path **before** relying on it in production.
